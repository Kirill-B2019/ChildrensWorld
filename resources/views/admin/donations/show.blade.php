<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Donation {{ $intent->external_reference }}</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <p><strong>Amount:</strong> {{ $intent->amount }} {{ $intent->currency }}</p>
                <p><strong>Status:</strong> {{ $intent->status }}</p>
                <p><strong>Donor:</strong> {{ $intent->donor_name ?: '-' }} / {{ $intent->donor_email ?: '-' }}</p>
                <form method="POST" action="{{ route('admin.content.donations.status', $intent) }}" class="mt-4 flex items-center gap-3">
                    ...
                    <button class="rounded bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Update status</button>
                </form>
                @if($errors->any())<p class="mt-2 text-sm text-red-600">{{ $errors->first() }}</p>@endif
            </div>

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="text-lg font-semibold">Transactions / webhook payloads</h3>
                <div class="mt-4 space-y-4">
                    @forelse($intent->transactions as $tx)
                        <article class="rounded border p-3">
                            <p><strong>Status:</strong> {{ $tx->status }} | <strong>TX:</strong> {{ $tx->transaction_id ?: '-' }}</p>
